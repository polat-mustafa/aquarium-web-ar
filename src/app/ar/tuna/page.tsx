...
function TunaARContent() {
  ...
  // Set Tuna Fish as the active creature with working settings
  useEffect(() => {
    setActiveCreature({
      id: 'model-tuna-fish',
      type: 'fish',
      ...
      description: 'Experience the magnificent Tuna Fish in AR!',
      scale: 1.5,
      position: [0, 0, -3],
      animation: 'idle'
    });
    console.log('✅ Set Tuna Fish as active creature');
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
  ...
  // CRITICAL: Log when creature changes to debug disappearing
  useEffect(() => {
  ...
  // Show popup when recording stops
  const [wasRecording, setWasRecording] = useState(false);
  useEffect(() => {
    if (wasRecording && !isRecording) {
      console.log('🔄 Recording state changed from true to false - ensuring popup shows');
      const fallbackTimer = setTimeout(() => {
        if (!showSharePanel) {
          console.log('🚨 Popup not shown, triggering fallback popup');
  ...
  const handleRecordingComplete = (blob: Blob) => {
    ...
    setShowSharePanel(true);
    console.log('✅ Video recorded successfully, showing share panel');
  };
  ...
  return (
    <div className="w-full min-h-screen bg-gradient-to-b from-slate-900 via-blue-900 to-slate-900 relative">
          ...
          {activeCreature && showCreaturePopup ? (
            ...
            <>
              ...
              {!isCameraReady && !cameraError && (
                <div className="bg-gradient-to-br from-slate-900/98 via-blue-900/95 to-slate-900/98 backdrop-blur-2xl border border-cyan-500/30 text-white p-10 rounded-3xl text-center space-y-8 shadow-2xl relative overflow-hidden">
                  {/* Animated background pattern */}
                  <div className="absolute inset-0 opacity-10">
                    <div className="absolute top-0 left-0 w-full h-full bg-gradient-to-br from-cyan-400/20 via-transparent to-blue-500/20 animate-pulse"></div>
                  </div>
                  ...
                  <div className="relative z-10 space-y-6">
                    ...
                    <div className="relative w-24 h-24 mx-auto">
                      ...
                      <div className="absolute inset-3 border-4 border-transparent border-t-blue-400 rounded-full animate-spin animation-delay-150" style={{ animationDuration: '1s' }}></div>
                    ...
                    <div className="space-y-3">
                      ...
                      <p className="text-cyan-200 text-lg font-medium">Preparing Tuna Fish</p>
                    </div>
      ...
      <SharePanel
        ...
        creatureName={activeCreature?.name || 'tuna-fish'}
      />
